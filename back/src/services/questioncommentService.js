import { Questioncomment } from '../db';
...
class questioncommentService {
  static async addComment({ board_id, user_id, name, content }) {
    ...
    const createdNewComment = await Questioncomment.createComment({ newComment });
    createdNewComment.errorMessage = null;
    return createdNewComment;
  }

  static async getComment({ comment_id }) {
    const comment = await Questioncomment.findById({ comment_id });
  ...
  static async getCommentsById({ user_id }) {
    const comments = await Questioncomment.findByUserId({ user_id });
    return comments;
  }

  // 게시글의 모든 댓글
  static async getCommentsByBoardId({ board_id }) {
    const comments = await Questioncomment.findByBoardId({ board_id });
    return comments;
  }

  // 수정
  static async setComment({ comment_id, toUpdate }) {
    let comment = await Questioncomment.findById({ comment_id });
    ...
    comment = await Questioncomment.update({ comment_id, newValues });
  ...
  static async deleteComment({ comment_id }) {
    ...
    const deletedComment = await Questioncomment.delete({ comment_id });
    return deletedComment;
  }
}

export { questioncommentService };
